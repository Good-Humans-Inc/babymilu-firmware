...
void WifiStation::SetPreferredSsidForNextConnect(const std::string& ssid) {
    preferred_ssid_ = ssid;
    ESP_LOGI(TAG, "Preferred SSID for next connect set to: %s", preferred_ssid_.c_str());
}
...
void WifiStation::HandleScanResult() {
    ...
    if (!preferred_ssid_.empty()) {
        auto preferred = std::find_if(ssid_list.begin(), ssid_list.end(), [this](const SsidItem& item) {
            return item.ssid == preferred_ssid_;
        });
        if (preferred != ssid_list.end()) {
            ESP_LOGI(TAG, "Prioritizing preferred SSID for this scan: %s", preferred_ssid_.c_str());
            std::rotate(ssid_list.begin(), preferred, preferred + 1);
        } else {
            ESP_LOGW(TAG, "Preferred SSID not found in stored credentials: %s", preferred_ssid_.c_str());
        }
    }
