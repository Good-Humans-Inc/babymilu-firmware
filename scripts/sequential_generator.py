...
def generate_overlays(emotions, overlay_dir, diff_threshold=0, target_size=None, resized_files=None):
    ...
    for emotion, frames in files_to_use.items():
        ...
        # Generate overlays: frame1→frame2, frame2→frame3, frame3→frame4, frame4→frame5, ...
        # Sequential comparison where each frame is compared to the previous one
        for i in range(1, len(sorted_frame_nums)):
            frame_num_from = sorted_frame_nums[i - 1]  # Previous frame
            frame_num_to = sorted_frame_nums[i]  # Current frame
