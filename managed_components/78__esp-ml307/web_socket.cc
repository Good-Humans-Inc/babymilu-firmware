...
bool WebSocket::Connect(const char* uri) {
    ...
    // Keep this thread lean, but leave enough headroom for WebSocket framing
    // and the server-hello JSON parse that must happen before OpenAudioChannel returns.
    ...
    try {
        ...
        ESP_LOGE(TAG, "Failed to create WebSocket receive thread: %s", e.what());
        transport_->Disconnect();
        if (on_error_) {
            on_error_(e.code().value());
        }
        return false;
    }
    return true;
