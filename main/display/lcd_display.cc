...
void LcdDisplay::CreateBatteryOverlayMessage(const char* battery_icon, lv_color_t battery_color,
                                             const char* battery_state, const char* network)
{
    const char* safe_icon = (battery_icon != nullptr && strlen(battery_icon) > 0)
        ? battery_icon
        : FONT_AWESOME_BATTERY_EMPTY;
    const char* safe_state = (battery_state != nullptr) ? battery_state : "";
    const char* safe_network = (network != nullptr && strlen(network) > 0) ? network : "offline";

    ESP_LOGI("LcdDisplay", "CreateBatteryOverlayMessage called: state='%s', network='%s'",
             safe_state, safe_network);
    DisplayLockGuard lock(this);
    ...
    lv_obj_set_style_border_width(overlay_container_, 0, 0);
    lv_obj_set_style_pad_all(overlay_container_, 0, 0);
    lv_obj_align(overlay_container_, LV_ALIGN_TOP_MID, 0, 4);
    lv_obj_move_foreground(overlay_container_);

    overlay_bubble_ = lv_obj_create(overlay_container_);
    lv_obj_clear_flag(overlay_bubble_, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_style_radius(overlay_bubble_, 6, 0);
    lv_obj_set_scrollbar_mode(overlay_bubble_, LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_style_border_width(overlay_bubble_, 0, 0);
    lv_obj_set_style_pad_left(overlay_bubble_, 8, 0);
    lv_obj_set_style_pad_right(overlay_bubble_, 8, 0);
    lv_obj_set_style_pad_top(overlay_bubble_, 2, 0);
    ...
    lv_coord_t icon_width = lv_txt_get_width(safe_icon, strlen(safe_icon), fonts_.icon_font, 0);
    lv_coord_t network_width = lv_txt_get_width(safe_network, strlen(safe_network), fonts_.text_font, 0);
    lv_coord_t max_width = LV_HOR_RES * 62 / 100;
    lv_coord_t chip_width = network_width + icon_width + 32;
    if (chip_width > max_width) {
        chip_width = max_width;
    }
    if (chip_width < 54) {
        chip_width = 54;
    }
    lv_obj_set_width(overlay_bubble_, chip_width);
    lv_obj_set_height(overlay_bubble_, fonts_.text_font->line_height + 6);

    overlay_text_ = lv_label_create(overlay_bubble_);
    lv_label_set_text(overlay_text_, safe_network);
    lv_label_set_long_mode(overlay_text_, LV_LABEL_LONG_DOT);
    lv_obj_set_width(overlay_text_, chip_width - icon_width - 28);
    lv_obj_set_style_text_font(overlay_text_, fonts_.text_font, 0);
    lv_obj_set_style_text_color(overlay_text_, lv_color_hex(0x1F2933), 0);
    lv_obj_set_style_text_align(overlay_text_, LV_TEXT_ALIGN_RIGHT, 0);

    lv_obj_t* icon = lv_label_create(overlay_bubble_);
    lv_label_set_text(icon, safe_icon);
    lv_obj_set_style_text_font(icon, fonts_.icon_font, 0);
    lv_obj_set_style_text_color(icon, battery_color, 0);
