    ...
    try {
        ...
        if (sd_write_test_alternative_works_) {
            ...
            if (test_file) {
                ...
                sd_write_test_alternative_works_ = false;
                // Fall through to primary test below
            }
        }
        ...
        if (!sd_write_test_alternative_works_) {
            ...
            if (test_file) {
                ...
                if (free_heap < MIN_MEMORY_FOR_REMOUNT) {
                    ESP_LOGW(TAG, "Insufficient memory for remount (%zu < %zu). Skipping remount and trying alternative filename.", 
                             free_heap, MIN_MEMORY_FOR_REMOUNT);
                } else {
                    ...
                    if (remount_ret != ESP_OK) {
                        ...
                        if (remount_ret == ESP_ERR_NO_MEM) {
                            ESP_LOGE(TAG, "Remount failed due to memory exhaustion. Will not attempt remount again.");
                            // Mark that we should skip remount in future attempts
                        }
                    } else {
