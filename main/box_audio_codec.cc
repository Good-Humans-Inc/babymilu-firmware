...
BoxAudioCodec::BoxAudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    ESP_LOGI(TAG, "codec initialized with reference=%s", input_reference_ ? "true" : "false");
}
...
void BoxAudioCodec::SetOutputVolume(int volume) {
    ESP_ERROR_CHECK(esp_codec_dev_set_out_vol(output_dev_, MapUiToHardwareVolume(volume)));
...
void BoxAudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        ESP_ERROR_CHECK(esp_codec_dev_set_in_channel_gain(input_dev_, ESP_CODEC_DEV_MAKE_CHANNEL_MASK(0), 37.0));
    } else {
